...
def make_preview_image_file(font_config: FontConfig):
    ...
    image = Image.new('RGBA', (font_config.size * 28, font_config.size * 2 + font_config.line_height * 9), (255, 255, 255, 255))
    _draw_text(image, (font_config.size, font_config.size), '方舟像素字体 - 传承字形 / Ark Pixel Font - Inherited', font)
    _draw_text(image, (font_config.size, font_config.size + font_config.line_height), '我们度过的每个平凡的日常，也许就是连续发生的奇迹。', font)
    _draw_text(image, (font_config.size, font_config.size + font_config.line_height * 2), '我們度過的每個平凡的日常，也許就是連續發生的奇蹟。', font)
    _draw_text(image, (font_config.size, font_config.size + font_config.line_height * 3), '日々、私たちが過ごしている日常は、', font)
    _draw_text(image, (font_config.size, font_config.size + font_config.line_height * 4), '実は奇跡の連続なのかもしれない。', font)
    ...
    _draw_text(image, (font_config.size, font_config.size + font_config.line_height * 6), 'the quick brown fox jumps over a lazy dog.', font)
    _draw_text(image, (font_config.size, font_config.size + font_config.line_height * 7), '0123456789', font)
    _draw_text(image, (font_config.size, font_config.size + font_config.line_height * 8), '★☆☺☹♠♡♢♣♤♥♦♧☀☼♩♪♫♬☂☁⚓✈⚔☯', font)
